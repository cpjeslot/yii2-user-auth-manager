...
use app\modules\auth\controllers\AssignmentController;
        ...
        if (!Configs::authManager()->getRule($name)) {
            try {
                ...
                if ($rule instanceof Rule) {
                    ...
                    $this->addError('ruleName', Yii::t('auth', 'Invalid rule "{value}"', ['value' => $name]));
                }
            } catch (\Exception $exc) {
                $this->addError('ruleName', Yii::t('auth', 'Rule "{value}" does not exists', ['value' => $name]));
            }
        }
    }
